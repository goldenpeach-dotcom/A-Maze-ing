...
再帰的バックトラッカーを選択。
迷路生成のロジックが明確で、シンプルに実装できると考えた。除外条件（外壁、４２ブロック）行けるとこまで行く、だめなら戻って探すの繰り返しで全域木（スパニングツリー）を作成する。
PERFECT=False時はloopを作成するために、セルの壁３方向がふさがっているセル（bitで判断）を狙い撃ちして開ける、その際の除外条件には3X3マスの空白も加わる。小さいの迷路の場合はループが作られない場合があるため最後に最低元のループを確保する保険ロジックがある。
再帰を使うとpythonの回数制限（深さ）を避けるためスタックでリストを使った反復版にした。
迷路の中心点を計算し、42ブロックの中心点と合わせて設置する。除外条件は、入口出口、外壁、中心点と42ブロック位置がかぶる場合は、上下左右ずらして条件合致すれば設置する仕様。
...
モジュールとしてmazegenをbuildすることができます。詳しくはLICENCE.md,pyproject.tomlを参照ください。
...
python関連のwebポータルサイトやキュレーションサイト、翻訳サイトおよびAIをコードの使用方法例と解説・バグの原因調査・ドキュメント作成の補助などに使用した。
...
kohira
迷路生成アルゴリズム、回避判定の設計・デバッグについて
ターミナル表示のANSIエスケープコード周りの実装補助、エラーハンドリングの網羅的なテスト(境界値・異常系)の洗い出しにも使用した。
