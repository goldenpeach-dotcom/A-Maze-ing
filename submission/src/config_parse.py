...
class ConfigError(ValueError):
    """configファイルが無い・壊れている・内容が不正な場合に送出する例外。"""
...
def read_config_file(file_name: str) -> dict[str, str]:
    """configファイルを読み込み、KEY=VALUEの辞書にする。

    Args:
        file_name: 読み込むconfigファイルのパス。
...
def parse_config(file_name: str) -> Config:
    """configファイルを読み込み、検証した上でConfigにする。
...
def main() -> None:
    """このファイル単体での動作確認用(config.txtを読んで表示するだけ)。"""
    try:
