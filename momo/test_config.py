...
def test_invalid_seed_raises(tmp_path):
...
def test_invalid_width_raises(tmp_path, bad_value):
    content = VALID_CONFIG.replace("WIDTH=10", f"WIDTH={bad_value}")
    path = write_config(tmp_path, content)
    with pytest.raises(ValueError, match="WIDTH and HEIGHT must be valid integers"):
        parse_config(path)


def test_width_too_small_raises(tmp_path):
...
def test_entry_bad_format_raises(tmp_path):
...
def test_invalid_perfect_raises(tmp_path):
...
def test_empty_output_file_raises(tmp_path):
